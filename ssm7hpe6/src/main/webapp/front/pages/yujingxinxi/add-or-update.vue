<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">日期</view>
				<picker mode="date" :value="ruleForm.riqi" @change="riqiChange">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(51, 153, 153, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.riqi?ruleForm.riqi:"请选择日期"}}</view>
				</picker>
			</view>
												<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(51, 153, 153, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
			...
			<!-- 否 -->
				...
				<view :style='{"width":"160rpx","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}' class="title">预警内容</view>
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(51, 153, 153, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.yujingneirong" placeholder="预警内容"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yujingxinxi`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='yujingneirong'){
					this.ruleForm.yujingneirong = obj[o];
					this.ro.yujingneirong = true;
					continue;
					}
					if(o=='riqi'){
		...
		methods: {
			...
			async onSubmitTap() {
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`yujingxinxi`, this.ruleForm);
				}else{
					await this.$api.add(`yujingxinxi`, this.ruleForm);
				}
