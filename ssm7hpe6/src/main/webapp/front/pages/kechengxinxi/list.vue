<template>
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.kechengleixing?'tabActive':''" @tap="categoryClick(item.kechengleixing)">{{item.kechengleixing}}</view>
        </scroll-view>
...
<script>
	export default {
		...
		async onShow() {
			let res = await this.$api.list('kechengleixing', {page:1,limit:100});
